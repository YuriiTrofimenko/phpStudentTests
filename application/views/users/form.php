        ...
        <?php Box::begin(['header' => $this->title, 'type' => 'primary']) ?>
            <?php $form = ActiveForm::begin([
                ...
                <?= $form->field($model, 'role')->dropDownList(\app\models\User::roleOptions(), [
                    'prompt' => '',
                    'readOnly' => !Yii::$app->user->can('users/manage-role', $model->scenario == 'create' ? null : [$model->user->id])
                ]) ?>

                <?= $form->field($model, 'activity')->widget('kartik\widgets\SwitchInput', [
                    'readonly' => !Yii::$app->user->can('users/toggle', $model->scenario == 'create' ? [null] : [$model->user->id])
                ]) ?>
